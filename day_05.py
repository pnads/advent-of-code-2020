...
with open('input_files/input_day_05.txt', 'r') as f:
    boarding_passes = [line.rstrip("\n") for line in f.readlines()]

# My solution is crap and it didn't work.

# This is Sandy's. It's brilliant.

def decode_seat_id(encoded):
    mapping = {ord('R'): '1', ord('L'): '0', ord('F'): '0', ord('B'): '1'}
    binary = encoded.translate(mapping)
    return int(binary[0:7], 2) * 8 + int(binary[7:], 2)

def part1():
    print(f'Max ID = {max((decode_seat_id(line) for line in boarding_passes))}')
...
def part2():
    ...
    seat_ids = [decode_seat_id(bp) for bp in boarding_passes]

    seat_range = list(range(0, max(seat_ids)))

    missing_seats = set(seat_range) - set(seat_ids)

    my_seat = max(missing_seats)
